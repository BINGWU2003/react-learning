...
```mermaid
flowchart LR
  deps["输入依赖"] --> compare{"依赖变化？"}
  compare -->|"是"| calculate["重新计算"]
  compare -->|"否"| cache["复用缓存"]
  calculate --> cache
  cache --> ui["渲染结果"]
```
...
```tsx title="React：手动列出依赖"
const summary = useMemo(
  () => filterSortAndSum(products, query, order),
  [products, query, order],
);
```

```ts title="Vue：自动追踪响应式读取"
const summary = computed(() =>
  filterSortAndSum(products.value, query.value, order.value),
);
```
