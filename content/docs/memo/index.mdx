...
> **先直接计算：**小数组或简单算式通常不需要 `useMemo`。先像[派生状态](/derived-state/)示例一样在渲染期间直接计算，确认存在明显开销后再缓存。

**`v-memo` 不是对应 API：**Vue 的 `v-memo` 缓存模板子树，更接近组件渲染优化；与 React `useMemo` 直接对应的是 `computed`。
